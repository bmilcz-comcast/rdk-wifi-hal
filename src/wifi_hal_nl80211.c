...
int nl80211_switch_channel(wifi_radio_info_t *radio)
{
    ...
    hash_map_foreach(radio->interface_map, interface) {
        ...
        if (wifi_hal_is_mld_link_exists(&interface->u.ap.hapd)) {
            for (int i = 0; i < g_wifi_hal.num_radios; ++i) {
                ...
                hash_map_foreach(radio_it->interface_map, if_it_radio) {
                    if (wifi_hal_is_mld_link_exists(&if_it_radio->u.ap.hapd)) {
                        if (if_it_radio->u.ap.hapd.csa_in_progress) {
                            // We are skipping since this is either
                            // already on changing channel or this was
                            // done just to satisfy MLD channel switch
                            // requirement
                            wifi_hal_error_print("%s:%d: CSA active - skip\n", __func__, __LINE__);
                            continue;
                        }

                        wifi_hal_dbg_print("%s:%d interface: %s switch channel to %d\n", __func__,
                            __LINE__, if_it_radio->name, radio_it->oper_param.channel);

                        pthread_mutex_lock(&g_wifi_hal.hapd_lock);
                        ret = hostapd_switch_channel(&if_it_radio->u.ap.hapd, &csa_settings);
                        pthread_mutex_unlock(&g_wifi_hal.hapd_lock);
                        ...
                        if (ret != 0) {
                            wifi_hal_error_print(
                                ...
                                __func__, __LINE__, wifi_hal_get_mld_link_id(if_it_radio), i, ret,
                                if_it_radio->u.ap.hapd.csa_in_progress);
                            return ret;
                        }
                    }
                }
