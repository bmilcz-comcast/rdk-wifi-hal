...
int platform_pre_create_vap(wifi_radio_index_t index, wifi_vap_info_map_t *map)
{
    ...
    for (unsigned int i = 0; i < map->num_vaps; i++) {
        ...
        memcpy(vap->u.bss_info.mld_info.common_info.mld_addr, interface->vap_info.u.bss_info.mld_info.common_info.mld_addr, sizeof(mac_address_t));
        vap->u.bss_info.mld_info.common_info.mld_link_id = interface->vap_info.u.bss_info.mld_info.common_info.mld_link_id;
