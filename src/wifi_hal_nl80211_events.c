...
int process_global_nl80211_event(struct nl_msg *msg, void *arg)
{
    ...
    if(interface) {
      ...
      //are going to have vap_configured set to false.
        if ((interface->vap_configured || wiphy_idx_rx != -1) || wifi_hal_get_mld_link_id(interface) != MLD_INVALID_VALUE)
        {
            wifi_hal_dbg_print("%s:%d: event registerd - processing\n", __func__, __LINE__);
            do_process_drv_event(interface, gnlh->cmd, tb);
            return NL_SKIP;
...
skip:
    wifi_hal_dbg_print("%s:%d: Skipping event %d for interface (ifindex %d wdev 0x%llx)\n", 
        __func__, __LINE__,
        gnlh->cmd,
