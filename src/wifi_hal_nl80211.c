    ...
    while (interface != NULL) {
        if (interface->vap_info.u.bss_info.enabled && is_wifi_hal_vap_mesh_sta(interface->vap_info.vap_index) == false) {
            ...
            if (interface->vap_info.u.bss_info.mld_info.common_info.mld_enable == true) {
                ...
                //TODO: we have to start_bss again here, otherwise the
                //beacon_set won't be called twice which is important
                //for BPi to broadcast - try moving this logic to start_bss
                // to unify the "double start".
                start_bss(interface);
