    ...
    while (interface != NULL) {
        if (interface->vap_info.u.bss_info.enabled &&
            ...
            if (interface->vap_info.u.bss_info.mld_info.common_info.mld_enable == true) {
                ...
                if (nl80211_create_bridge(wifi_hal_get_interface_name(interface), interface->bridge) != 0) {
                    wifi_hal_error_print("%s:%d: Failed to add interface: %s to bridge %s\n", __func__, __LINE__,
                        wifi_hal_get_interface_name(interface), interface->bridge);
                    return -1;
                }
