                                              ...
                                              int *secondary_channel)
{
    ...
    chan = dfs_get_valid_channel(&interface->u.ap.iface, secondary_channel,
        oper_centr_freq_seg0_idx, oper_centr_freq_seg1_idx, DFS_AVAILABLE);
#else
...
int set_freq_and_interface_enable(wifi_interface_info_t *interface, wifi_radio_info_t *radio) {
    ...
    wifi_hal_info_print(
        "%s:%d name:%s freq:%d sec_chan:%d bandwidth:%d chan:%u, cf1:%d, ht_enabled:%d \n",
        __func__, __LINE__, interface->name, freq, sec_chan_offset, radio->oper_param.channelWidth,
        radio->oper_param.channel, cf1, ht_enabled);
