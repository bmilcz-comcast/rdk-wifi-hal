...
int nl80211_update_wiphy(wifi_radio_info_t *radio)
{
        ...
        if (interface->vap_info.u.bss_info.enabled && is_wifi_hal_vap_mesh_sta(interface->vap_info.vap_index) == false) {
            ...
            if (wifi_hal_is_mld_enabled(interface)) {
                if (teardown_mlo_vap(interface)) {
                    wifi_hal_error_print("%s:%d: Failed to teardown MLO VAP for interface %s\n", __func__, __LINE__, wifi_hal_get_interface_name(interface));
                    return -1;
                }
            } else {
                if (reload_interface(interface)) {
                    wifi_hal_error_print("%s:%d: Failed to reload interface %s\n", __func__, __LINE__, wifi_hal_get_interface_name(interface));
                    return -1;
                }
    ...
    //? Also - above causes for all interfaces to get disabled. - why do we do this for last interface ?
    if (!radio->configured) {
        ...
        if (interface->vap_info.u.bss_info.enabled && is_wifi_hal_vap_mesh_sta(interface->vap_info.vap_index) == false) {
            ...
            if (wifi_hal_is_mld_enabled(interface)) {
                if (setup_mlo_vap(interface, &interface->vap_info)) {
                    wifi_hal_error_print("%s:%d: Failed to setup MLO VAP interface %s\n", __func__, __LINE__, wifi_hal_get_interface_name(interface));
                    return -1;
                }
            } else {
                if (restart_interface(interface) < 0) {
                    wifi_hal_error_print("%s:%d: Failed to restart interface %s\n", __func__, __LINE__, wifi_hal_get_interface_name(interface));
                    return -1;
                }
